                ...
                {!loading && !error && verses.length > 0 && (() => {
                    ...
                    for (let i = 1; i <= 15; i++) {
                        ...
                        const wordSpans = lines[i].map((w, idx) => {
                            ...
                            const isHighlighted = highlightState && isVerseInRange(w.verse_key, highlightState.startVk, highlightState.endVk);
...
export default function QuranPageViewer({ selectedJuz, surahName, verseStart, inline = false, onAyahClick = null }) {
    ...
    const [highlightState, setHighlightState] = useState({ startVk: null, L_start: null, endVk: null });

    const surahNum = surahName ? SURAH_MAPPING[surahName] : null;
    const startVerseKey = surahNum && verseStart ? `${surahNum}:${verseStart}` : null;

    console.log('QuranPageViewer props:', { selectedJuz, surahName, verseStart, inline });
    console.log('Surah mapping result:', surahNum);

    useEffect(() => {
        if (!startVerseKey) return;
        if (highlightState.startVk !== startVerseKey) {
            setHighlightState({ startVk: startVerseKey, L_start: null, endVk: null });
        }
    ...
    useEffect(() => {
        ...
        let newEndVk = highlightState.endVk;
        let changed = false;
        ...
        if (newL_start !== null && newEndVk === null) {
            const L_target = newL_start + 7; // 8 lines total
            const currentMinLine = (pageNumber - 1) * 15 + 1;
            const currentMaxLine = pageNumber * 15;
            
            if (L_target >= currentMinLine && L_target <= currentMaxLine) {
                let targetLocalLine = L_target - (pageNumber - 1) * 15;
                let wordsOnLine = verses.flatMap(v => v.words || []).filter(w => parseInt(w.line_number, 10) === targetLocalLine);
                
                while (wordsOnLine.length === 0 && targetLocalLine > 1) {
                    targetLocalLine--;
                    wordsOnLine = verses.flatMap(v => v.words || []).filter(w => parseInt(w.line_number, 10) === targetLocalLine);
                }
                
                if (wordsOnLine.length > 0) {
                    newEndVk = wordsOnLine[wordsOnLine.length - 1].verse_key;
                    changed = true;
                }
            }
        }
        ...
        if (changed) {
            setHighlightState({ startVk: startVerseKey, L_start: newL_start, endVk: newEndVk });
        }
    }, [verses, pageNumber, startVerseKey, highlightState]);
