...
export default function Edit({ silabus, mapels, jenjangs, kelas }) {
    const { data, setData, put, processing, errors } = useForm({
        mapel_id: silabus?.mapel_id || '',
        jenjang_id: silabus?.jenjang_id || '',
        kelas_id: silabus?.kelas_id || '',
        kurikulum: silabus?.kurikulum || 'Merdeka',
        semester: silabus?.semester || 'Ganjil',
        kode: silabus?.kode || '',
        standar_kompetensi: silabus?.standar_kompetensi || '',
        kompetensi: silabus?.kompetensi || '',
        materi: silabus?.materi || '',
        alokasi_waktu: silabus?.alokasi_waktu || '',
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('silabus.update', silabus.id));
    };

    return (
        <MainLayout>
            <Head title="Edit Silabus" />
            ...
            <div className="max-w-3xl mx-auto space-y-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h2 className="text-3xl font-bold tracking-tight text-foreground">Edit Silabus</h2>
                        <p className="text-muted-foreground">Perbarui standar kompetensi dan materi pembelajaran.</p>
                    </div>
                </div>
